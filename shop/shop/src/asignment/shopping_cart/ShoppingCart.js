...
async function orderPizza(id, pizzas) {
  return makeHttpRequest(
    "http://127.0.0.1:3001/insertPizzaOrder",
...
async function insertOrder(orderedId, pizzas) {
  return makeHttpRequest(
    "http://127.0.0.1:3001/insertOrder",
    { "Content-Type": "application/json" },
    {
      pizzaOrderId: orderedId,
      pizzaIds: pizzas,
    }
  );
}

function ShoppingCart({ info, setInfo }) {
  const {
    id,
    pizzaIds,
    name_eng,
  ...
  //useEffect(() => {
  //  orderPizza(id, 10000, "2023-01-01").then((req) => {
  //    console.log("주문 아이디: ", req);
  //  });
  //}, []);

  const items = info.pizzaIds;
    ...
    return (
      <div className="shopping_item" ref={shoppingItem}>
        ...
        <div className="change">
          <div className="blue_btn">// ref={() => changeBtn}> 변경저장</div>
        </div>
  ...
  return (
    <div className="shopping_cart">
      ...
      <div className="shopping_list">
        ...
        <Link to="/pizza">
          <div
            className="order blue_btn"
            ref={() => orderBtn}
            onClick={() =>
              orderPizza(id, pizzaIds).then((req) => {
                setInfo({ ...info, orderedId: req });
                insertOrder(req, pizzaIds);
              })
            }
          >
            주문하기
