...
async function selectOrderHistory(orderId, userId) {
  console.log("-+-" + userId);
  return makeHttpRequest(
    "http://127.0.0.1:3001/searchOrderHistory",
...
function OrderHistory(props) {
  ...
  useEffect(() => {
    selectOrderHistory(props.orderedId, props.userId).then((res) => {
      ...
      <div className=".post-container">
        <div className="info-container">
          ...
          <div
            className="info-list"
            onClick={() => {
              !props.showReviewClick
                ? props.handleShowReviewClick(true)
                : props.handleShowReviewClick(false);
            }}
          >
            주문메뉴 : {arrayContentsToString(menuNames)}
          </div>
          <div className="info-list">
            메뉴 아이디 : {arrayContentsToString(menuIds)}
          </div>
