...
async function enrollReview(userId, pizzaId, rating, contents) {
  ...
  return makeHttpRequest(
    "http://127.0.0.1:3001/enrollReview",
  ...
  return (
    <Fragment>
      <div className="enter-review">
        ...
        <label for="reviewLabel">
          메뉴명 :{" "}
          {props.menu.menuNames.length > index
            ? props.menu.menuNames[index]
            : " 리뷰 등록할 메뉴 없음"}{" "}
          - Review :
        </label>
        <input
        ...
        <button
          onClick={() => {
            setIndex(index + 1);
            setStarRating(0);
            !props.click || props.menu.menuNames.length > index + 1
              ? props.handleClick(true)
              : props.handleClick(false);
            const userId = props.userId;
            let pizzaId = props.pizzas[index].id;
