...
async function selectUser(userId) {
  return makeHttpRequest(
    "http://127.0.0.1:3001/searchUserInfo",
...
async function updateUserAddress(userId, address) {
  return makeHttpRequest(
    "http://127.0.0.1:3001/updateUserInfo",
    { "Content-Type": "application/json" },
    ...
    selectUser(userId).then((res) => {
      if (edit == 0) {
        setUser({
          ...
          birthday:
            res[0].birthday +
            (res[0].gender == 1 || res[0].gender == 3 ? " (남성)" : " (여성)"),
        });
      }
      console.log(setUser);
    });
      ...
      <div className="subpage">
        ...
        <div>
          <input
            ...
            id="password"
            name="password"
            placeholder="비밀번호(8~16자리 영문/ 순자 조합)"
          />
        </div>
        <div>
          <input
        ...
        <div>
          <button className="button" id="left-button" onClick={() => {}}>
            취소
          </button>
